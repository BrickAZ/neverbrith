...
local function test_xml_registers_needletick_item_text_and_devil_pool()
    ...
    assertTruthy(enItems:find('name="Void Needle"', 1, true), "en item xml should register Needletick")
    assertTruthy(zhItems:find('name="虚空针尖"', 1), "zh item xml should register Needletick")
    assertTruthy(enItems:find('description="Next time, spit in the trash..."', 1, true), "en pickup text should match design")
