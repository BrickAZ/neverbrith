-- Execute the real entrypoint with registration as a boundary sentinel.
local cases = {
    { label = "missing" },
...
for _, case in ipairs(cases) do
    ...
    env.RegisterMod = function() registrations = registrations + 1; error("REGISTRATION_BOUNDARY") end
    env.include = function() includes = includes + 1; error("unexpected include") end
    env.Isaac = {
    ...
    for attempt = 1, 2 do
        ...
        if case.supported then
            ...
            assert(ok, case.label .. " should stop cleanly before registration: " .. tostring(err))
        end
    end
    assert(includes == 0, case.label .. " must not include before boundary")
    assert(registrations == (case.supported and 2 or 0), case.label .. " registration count")
